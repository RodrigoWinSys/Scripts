...
def execute():
    ...
    os.chdir(script_dir)
    #os.chdir('..\..\Inventario')
    inventario=pd.read_excel(r'C:\Users\Rodrigo Rodriguez\OneDrive - Win Systems Group\Funnel\Inventario\Inventario Mexico.xlsx',header=4,sheet_name='Inventario')
    
    
    df.rename(columns={'Slot': 'Serie'}, inplace=True)
    df['Serie'] = df['Serie'].str.replace('GST', '')
    df['Serie'] = pd.to_numeric(df['Serie'], errors='coerce')
    df=df.merge(inventario,how='left',left_on='Serie',right_on='Serie')
    print (df)
    ...
    for i in ['Grupo','Casino','Slot ID','Match online','Serie','Cabinet',
                     ...
                     'TGW CI','TGG NW','TGW ARP','Slot_x','time_x','Estatus','Modo',
    ...
    try:      
        df=df[['Grupo','Casino','Slot ID','Match online','Serie','Cabinet',
                         'Modelo','Mix','RTP TH','Hold TH','Jackpot','Tipo JP','Area',
                         'Isla','Posicion','Pais','Estado','Ciudad','Muncipio','TG SP',
                         'TGW CI','TGW NW','TGW ARP','Slot_x','time_x','Estatus','Modo',
                         '% Participacion','Online','ON','Positions Approached',
                         'Positions Denied','Positions Confirmed',
                         'Infraestructure Needed','Infrastructure Exist','Scheduled',
                         'Positions Conected','Coments','# Isla','Connected',
